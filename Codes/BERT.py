...
from Data_prepare import make_data_for_pretrain, make_data_for_pretrain_pmhc
import warnings
...
class Embedding(nn.Module):
    def __init__(self, vocab_size, maxlen=55, d_model=768):
...
class DualTaskBERT(nn.Module):
    ...
    def __init__(self, vocab_size, d_model=768, n_layers=8):
        ...
        self.fc2 = nn.Linear(d_model, vocab_size, bias=False)
        self.fc2.weight = embed_weight
        self.fc_segment = nn.Linear(d_model, 2)

    def forward(self, input_ids, masked_pos, segment_labels=None):
        ...
        logits_segment = self.fc_segment(output)
        return logits_lm, logits_segment
...
def create_parser():
    ...
    parser.add_argument("--model_path",dest="model_path",type=str, help="the dir to save model.", required=True)
    ...
    parser.add_argument("--pMHC",dest="pMHC",action="store_true",help='whether include pMHC')
    parser.add_argument("--seg",dest="seg",action="store_true",help='whether include segment classification')
...
def main(args):
    ...
    if args.pMHC:
        dataset, validate_dataset,vocab_size = make_data_for_pretrain_pmhc(args.data_path, args.maxlen, max_pred=10, seg=args.seg)
    else:
    ...
    if args.seg:
        model = DualTaskBERT(vocab_size)
    else:
        model = BERT(vocab_size)
   
    criterion = nn.CrossEntropyLoss()
    criterion_segment = nn.CrossEntropyLoss() if args.seg else None
    ...
    logging_save_path = os.path.join(model_save_dir,'training_log.log')
    ...
    print("logger init, ready to run")
    ...
    for epoch in range(args.epoch):
        ...
        for batch in tqdm(data_loader,total=len(data_loader)):
            ...
            if args.seg:
                input_ids, masked_tokens, masked_pos, segment_labels = batch
                logits_lm, logits_segment = model(input_ids, masked_pos)
                loss_lm = criterion(logits_lm.view(-1, vocab_size), masked_tokens.view(-1))
                
                segment_mask = (segment_labels != -1)  # 创建掩码，只保留有效的 segment 标签
                valid_segment_labels = segment_labels[segment_mask]
                valid_logits_segment = logits_segment.view(-1, 2)[segment_mask.view(-1)]
                loss_segment = criterion_segment(valid_logits_segment, valid_segment_labels)
                
                loss = loss_lm + loss_segment
                total_loss_lm += loss_lm.item()
                total_loss_segment += loss_segment.item()
        ...
        epoch_loss_lm = total_loss_lm / len(data_loader)
        epoch_loss_segment = total_loss_segment / len(data_loader) if args.seg else 0
        
        info_train = f"epoch:{epoch + 1}, done! total_loss:{epoch_loss}, loss_lm:{epoch_loss_lm}, loss_seg:{epoch_loss_segment}"
        ...
        loss_v_segment = 0
        with torch.no_grad():
            for batch in tqdm(validate_dataloader, total=len(validate_dataloader)):
                if args.seg:
                    input_ids, masked_tokens, masked_pos, segment_labels = batch
                    logits_lm, logits_segment = model(input_ids, masked_pos)
                    loss_lm = criterion(logits_lm.view(-1, vocab_size), masked_tokens.view(-1))
                    
                    segment_mask = (segment_labels != -1)
                    valid_segment_labels = segment_labels[segment_mask]
                    valid_logits_segment = logits_segment.view(-1, 2)[segment_mask.view(-1)]
                    loss_segment = criterion_segment(valid_logits_segment, valid_segment_labels)
                    loss = loss_lm + loss_segment
                    loss_v_lm += loss_lm.item()
                    loss_v_segment += loss_segment.item()
                else:
                    input_ids, masked_tokens, masked_pos = batch
                    _, logits_lm = model(input_ids, masked_pos)
                    loss_lm = criterion(logits_lm.view(-1, vocab_size), masked_tokens.view(-1))
            ...
            epoch_loss_v = loss_v / len(validate_dataloader)
            epoch_loss_v_lm = loss_v_lm / len(validate_dataloader)
            epoch_loss_v_segment = loss_v_segment / len(validate_dataloader) if args.seg else 0

        info_test = f"loss in validate_dataset: total_loss:{epoch_loss_v}, loss_lm:{epoch_loss_v_lm}, loss_seg:{epoch_loss_v_segment}"
        accelerator.print(info_test)
    ...
    print(f"Done, the model saved in directory {args.model_path}")
...
# python BERT.py --data_path="/hpc-cache-pfs/home/chenjunwei/script/GRATCR/Data/pMHC/pMHC_36W_valid.csv" --model_path="/hpc-cache-pfs/home/chenjunwei/script/GRATCR/model/pMHC_BERT/pmhcbert_1224.pt" --epoch=30 --maxlen=55 --pMHC
