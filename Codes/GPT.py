...
def create_parser():
    ...
    parser.add_argument("--train_samples",dest="train_samples",type=int,help="max samples to train GPT",default=999999999,required=False)
    ...
    parser.add_argument("--vocab_path",dest="vocab_path",type=str,default=None,help="The vocab file in .csv format.",required=False)
...
def main(args):
    ...
    #dataset, validate_dataset, vocab_size = make_data_for_gpt_pretrain(args.data_path, args.maxlen, args.vocab_path)
    dataset, validate_dataset, vocab_size = make_data_for_gpt_pretrain(args.data_path, args.maxlen, args.train_samples, None)
    ...
    for epoch in range(args.epoch):
        model.train()
        loss = 0
        for input_ids, output_labels, mask_for_loss in tqdm(data_loader,total=len(data_loader)):
            optimizer.zero_grad()
        ...
        info_train = f"epoch:{epoch + 1},done! loss:{loss_epoch}"
        ...
        loss_v = 0
        with torch.no_grad():
            for input_ids, output_labels, mask_for_loss in tqdm(validate_dataloader,total=len(validate_dataloader)):
                _, logits_lm = model(input_ids)
                loss_lm = criterion(logits_lm.transpose(1, 2), output_labels) 
                loss_lm = loss_lm * mask_for_loss
                loss_lm = (loss_lm.float()).mean()
        ...
        loss_epoch_v = loss_v / len(validate_dataloader)
        info_test = f"loss in validate_dataset:{loss_epoch_v}"
